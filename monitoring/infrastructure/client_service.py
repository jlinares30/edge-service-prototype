    ...
    def _background_sync_loop(self):
        ...
        while True:
            try:
                ...
                unsynced = list(TelemetryRecordModel.select().where(TelemetryRecordModel.is_synced == False).order_by(TelemetryRecordModel.created_at.asc()))
    ...
    def _sync_record(self, record) -> bool:
        ...
        try:
            ...
            if response.status_code == 201:
                ...
                print(f"[EDGE-TO-CLOUD FAILURE] Backend returned status code {response.status_code} for device {record.device_id}")
                return False
